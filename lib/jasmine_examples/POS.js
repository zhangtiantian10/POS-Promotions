...
POS.prototype.sameCodeCount = function(jsonData){
  ...
  for(var i = 0 ; i < jsonData.length; i++) {
    ...
    if(codeArray.length === 1) {
      codeCounts = this.a(codeArray[0], codeCounts, 1);
    }
    else {
      codeCounts = this.a(codeArray[0], codeCounts, parseInt(codeArray[1]));
    }
  }
  return codeCounts;
}

POS.prototype.a = function(code, codeCounts, count) {
  for(var i = 0 ; i < codeCounts.length ; i++) {
    if(code === codeCounts[i].barcode) {
      codeCounts[i].count += count;
      break;
    }
  }

  if(i === codeCounts.length) {
    codeCounts.push({barcode : code, count : count});
  }
...
POS.prototype.addAttributes = function(item, flag, count) {
  return item.name + ' '
        ...
        + count + item.unit + ' '
         + parseFloat(item.price * count).toFixed(2) + '元' + flag;
}
